...
ges_audio_source_create_element (GESTrackElement * trksrc)
{
  ...
  vbin =
      ...
      ("audioconvert ! scaletempo ! audioconvert ! audioconvert name=convert ! audiorate skip-to-first=true ! audioconvert ! audioresample ! audioconvert ! volume name=v ! capsfilter name=audio-track-caps-filter",
      TRUE, NULL);
