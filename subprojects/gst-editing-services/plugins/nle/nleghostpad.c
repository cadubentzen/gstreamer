...
/*
 * NleGhostPad — GstGhostPad subclass that carries all the NLE-specific
 * data formerly stored via gst_pad_set_element_private().
 *
 * The internal proxy pad retrieves this data by navigating back to its
 * ghost pad owner via gst_proxy_pad_get_internal(), which is always safe
 * because the proxy pad's lifetime is bounded by the ghost pad's.
 */
...
control_internal_pad (NleGhostPad * nle_ghost)
{
  ...
  GST_LOG_OBJECT (nle_ghost,
      "overriding ghostpad's internal pad function");
