...
gst_plugin_feature_class_init (GstPluginFeatureClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_RANK,
      g_param_spec_enum ("rank", "Rank", "The rank of the plugin feature",
          GST_TYPE_RANK, GST_RANK_NONE,
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_RANK:
      gst_plugin_feature_set_rank (feature, g_value_get_enum (value));
      break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_RANK:
      g_value_set_enum (value, gst_plugin_feature_get_rank (feature));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
