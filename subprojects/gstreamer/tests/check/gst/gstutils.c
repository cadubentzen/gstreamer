...
GST_START_TEST (test_regression)
{
  ...
  for (i = 0; i < G_N_ELEMENTS (regression_times); i++) {
    fail_unless (gst_calculate_linear_regression (regression_times[i].v, NULL, regression_times[i].n,
    ...
    fail_unless (internal == regression_times[i].expect_internal,
        "Regression params %d fail. internal %" G_GUINT64_FORMAT
        " != expected %" G_GUINT64_FORMAT, i, internal,
        regression_times[i].expect_internal);
    /* Rate must be within 1% tolerance */
    expect_rate = ((gdouble) (regression_times[i].expect_num) / regression_times[i].expect_denom);
    ...
    fail_unless ((expect_rate - rate) >= -0.1 && (expect_rate - rate) <= 0.1,
        ...
        i, expect_rate, rate);
    fail_unless (external >= regression_times[i].expect_external * 0.99 &&
        external <= regression_times[i].expect_external * 1.01,
        "Regression params %d fail. external %" G_GUINT64_FORMAT
        " != expected %" G_GUINT64_FORMAT, i, external,
        regression_times[i].expect_external);
  }
}
